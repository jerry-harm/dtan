            ...
            <>
                <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-2 text-center">
                        {!customAmount &&
                            (satsAmounts).map(a => (
                                <div key={a} className={classNames("rounded-full", a === amount ? "bg-neutral-600" : "bg-neutral-800")} onClick={() => setAmount(a)}>
                                    {formatSats(a)}
                                </div>
                            ))}
                        <div onClick={() => setCustomAmount(s => !s)} className={classNames("rounded-full", customAmount ? "bg-neutral-600" : "bg-neutral-800")}>
                            Custom
                        </div>
                    </div>
                ...
                {svc && (svc.maxCommentLength > 0 || svc.canZap) && (
                    <div className="flex flex-col gap-2">
                        ...
                        <textarea className="w-full" placeholder="Nice!" value={comment} onChange={e => setComment(e.target.value)} />
                    </div>
                )}
